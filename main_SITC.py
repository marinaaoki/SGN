...
def main():
    ...
    model = SGN(40, args.dataset, args.seg, args)
    ...
    save_path = os.path.join(output_dir, args.network, "TC2-1", "f3")
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    ntu_checkpoint = "/u/home/aoma/gr/SGN/results/NTU_SRID/SGN/1_best.pth"
    sitc_checkpoint = "/u/home/aoma/gr/SGN/results/SITC_SAR/SGN/TC2-1/f3/0_best.pth"
    checkpoint = osp.join(save_path, '%s_best.pth' % args.case)

    earlystop_cnt = 0
    csv_file = osp.join(save_path, '%s_%s_log.csv' % (args.dataset, args.case))
    log_res = list()
    ...
    if args.train == 1:
        if args.transfer == 1:
            model.load_state_dict(torch.load(ntu_checkpoint)['state_dict'])
            model.fc = nn.Linear(model.fc.in_features, args.num_classes).cuda()
            print(model)
    ...
    model = model.cuda()
    if args.dataset.startswith('PP-SITC'):
        checkpoint = sitc_checkpoint
    if args.dataset.endswith('SITC_SRID'):
        test_topk(test_loader, model, checkpoint, lable_path, pred_path)
    else:
...
def train_topk(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        accs = accuracy(output.data, target, topk=(1, 3))
        ...
        if (i + 1) % args.print_freq == 0:
            print('Epoch-{:<3d} {:3d} batches\t'
                  ...
                  'top-3 accu {acc_top5.val:.3f} ({acc_top5.avg:.3f})'.format(
...
def validate_topk(val_loader, model, criterion):
    losses = AverageMeter()
    acces_top1 = AverageMeter()
    acces_top5 = AverageMeter()
...
def test_topk(test_loader, model, checkpoint, lable_path, pred_path):
    acces_top1 = AverageMeter()
    acces_top5 = AverageMeter()
    ...
    for i, (inputs, target) in enumerate(test_loader):
        if i == 0:
            summary = torchsummary.summary(model, inputs, device="cuda")
            print(summary)
            # save summary to file
            with open('PP-SITC_SRID_summary.txt', 'w') as f:
                f.write(str(summary))
        ...
        accs = accuracy(output.data, target.cuda(non_blocking=True), topk=(1, 3))
        acces_top1.update(accs[0][0], inputs.size(0))
        acces_top5.update(accs[1][0], inputs.size(0))
    ...
    print('Test: top-1 accuracy {:.3f}, top-3 accuracy {:.3f} time: {:.2f}s'
...
def test(test_loader, model, checkpoint, lable_path, pred_path):
    ...
    for i, (inputs, target) in enumerate(test_loader):
        ...
        print(summary)
        # save summary to file
        with open('PP-SITC_SAR_summary.txt', 'w') as f:
            f.write(str(summary))
        with torch.no_grad():
            output = model(inputs.cuda())
            output = output.view((-1, inputs.size(0)//target.size(0), output.size(1)))
